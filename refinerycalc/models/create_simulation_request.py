# coding: utf-8

"""
    Refinery Calc API Documentation

    Integrate the powerful Refinery Calc Engine into your process using this API.  # noqa: E501

    OpenAPI spec version: 1.0
    Contact: support@refinerycalc.com.com
    Generated by: https://github.com/swagger-api/swagger-codegen.git
"""

import pprint
import re  # noqa: F401

import six


class CreateSimulationRequest(object):
    """NOTE: This class is auto generated by the swagger code generator program.

    Do not edit the class manually.
    """
    """
    Attributes:
      swagger_types (dict): The key is attribute name
                            and the value is attribute type.
      attribute_map (dict): The key is attribute name
                            and the value is json key in definition.
    """
    swagger_types = {
        'name': 'str',
        'refineries': 'list[int]',
        'data_source': 'DataSource',
        'is_crude_valuation': 'bool',
        'crude_evaluation_mode': 'CrudeEvaluationMode',
        'is_time_series': 'bool',
        'time_series_days': 'list[TimeSeriesDay]',
        'api_simulation': 'bool',
        'is_time_travel': 'bool',
        'time_travel_start_date': 'str',
        'time_travel_end_date': 'str',
        'custom_aggregate_refineries': 'list[int]'
    }

    attribute_map = {
        'name': 'name',
        'refineries': 'refineries',
        'data_source': 'dataSource',
        'is_crude_valuation': 'isCrudeValuation',
        'crude_evaluation_mode': 'crudeEvaluationMode',
        'is_time_series': 'isTimeSeries',
        'time_series_days': 'timeSeriesDays',
        'api_simulation': 'apiSimulation',
        'is_time_travel': 'isTimeTravel',
        'time_travel_start_date': 'timeTravelStartDate',
        'time_travel_end_date': 'timeTravelEndDate',
        'custom_aggregate_refineries': 'customAggregateRefineries'
    }

    def __init__(self, name=None, refineries=None, data_source=None, is_crude_valuation=None, crude_evaluation_mode=None, is_time_series=None, time_series_days=None, api_simulation=None, is_time_travel=None, time_travel_start_date=None, time_travel_end_date=None, custom_aggregate_refineries=None):  # noqa: E501
        """CreateSimulationRequest - a model defined in Swagger"""  # noqa: E501
        self._name = None
        self._refineries = None
        self._data_source = None
        self._is_crude_valuation = None
        self._crude_evaluation_mode = None
        self._is_time_series = None
        self._time_series_days = None
        self._api_simulation = None
        self._is_time_travel = None
        self._time_travel_start_date = None
        self._time_travel_end_date = None
        self._custom_aggregate_refineries = None
        self.discriminator = None
        if name is not None:
            self.name = name
        if refineries is not None:
            self.refineries = refineries
        if data_source is not None:
            self.data_source = data_source
        if is_crude_valuation is not None:
            self.is_crude_valuation = is_crude_valuation
        if crude_evaluation_mode is not None:
            self.crude_evaluation_mode = crude_evaluation_mode
        if is_time_series is not None:
            self.is_time_series = is_time_series
        if time_series_days is not None:
            self.time_series_days = time_series_days
        if api_simulation is not None:
            self.api_simulation = api_simulation
        if is_time_travel is not None:
            self.is_time_travel = is_time_travel
        if time_travel_start_date is not None:
            self.time_travel_start_date = time_travel_start_date
        if time_travel_end_date is not None:
            self.time_travel_end_date = time_travel_end_date
        if custom_aggregate_refineries is not None:
            self.custom_aggregate_refineries = custom_aggregate_refineries

    @property
    def name(self):
        """Gets the name of this CreateSimulationRequest.  # noqa: E501


        :return: The name of this CreateSimulationRequest.  # noqa: E501
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name):
        """Sets the name of this CreateSimulationRequest.


        :param name: The name of this CreateSimulationRequest.  # noqa: E501
        :type: str
        """

        self._name = name

    @property
    def refineries(self):
        """Gets the refineries of this CreateSimulationRequest.  # noqa: E501


        :return: The refineries of this CreateSimulationRequest.  # noqa: E501
        :rtype: list[int]
        """
        return self._refineries

    @refineries.setter
    def refineries(self, refineries):
        """Sets the refineries of this CreateSimulationRequest.


        :param refineries: The refineries of this CreateSimulationRequest.  # noqa: E501
        :type: list[int]
        """

        self._refineries = refineries

    @property
    def data_source(self):
        """Gets the data_source of this CreateSimulationRequest.  # noqa: E501


        :return: The data_source of this CreateSimulationRequest.  # noqa: E501
        :rtype: DataSource
        """
        return self._data_source

    @data_source.setter
    def data_source(self, data_source):
        """Sets the data_source of this CreateSimulationRequest.


        :param data_source: The data_source of this CreateSimulationRequest.  # noqa: E501
        :type: DataSource
        """

        self._data_source = data_source

    @property
    def is_crude_valuation(self):
        """Gets the is_crude_valuation of this CreateSimulationRequest.  # noqa: E501


        :return: The is_crude_valuation of this CreateSimulationRequest.  # noqa: E501
        :rtype: bool
        """
        return self._is_crude_valuation

    @is_crude_valuation.setter
    def is_crude_valuation(self, is_crude_valuation):
        """Sets the is_crude_valuation of this CreateSimulationRequest.


        :param is_crude_valuation: The is_crude_valuation of this CreateSimulationRequest.  # noqa: E501
        :type: bool
        """

        self._is_crude_valuation = is_crude_valuation

    @property
    def crude_evaluation_mode(self):
        """Gets the crude_evaluation_mode of this CreateSimulationRequest.  # noqa: E501


        :return: The crude_evaluation_mode of this CreateSimulationRequest.  # noqa: E501
        :rtype: CrudeEvaluationMode
        """
        return self._crude_evaluation_mode

    @crude_evaluation_mode.setter
    def crude_evaluation_mode(self, crude_evaluation_mode):
        """Sets the crude_evaluation_mode of this CreateSimulationRequest.


        :param crude_evaluation_mode: The crude_evaluation_mode of this CreateSimulationRequest.  # noqa: E501
        :type: CrudeEvaluationMode
        """

        self._crude_evaluation_mode = crude_evaluation_mode

    @property
    def is_time_series(self):
        """Gets the is_time_series of this CreateSimulationRequest.  # noqa: E501


        :return: The is_time_series of this CreateSimulationRequest.  # noqa: E501
        :rtype: bool
        """
        return self._is_time_series

    @is_time_series.setter
    def is_time_series(self, is_time_series):
        """Sets the is_time_series of this CreateSimulationRequest.


        :param is_time_series: The is_time_series of this CreateSimulationRequest.  # noqa: E501
        :type: bool
        """

        self._is_time_series = is_time_series

    @property
    def time_series_days(self):
        """Gets the time_series_days of this CreateSimulationRequest.  # noqa: E501


        :return: The time_series_days of this CreateSimulationRequest.  # noqa: E501
        :rtype: list[TimeSeriesDay]
        """
        return self._time_series_days

    @time_series_days.setter
    def time_series_days(self, time_series_days):
        """Sets the time_series_days of this CreateSimulationRequest.


        :param time_series_days: The time_series_days of this CreateSimulationRequest.  # noqa: E501
        :type: list[TimeSeriesDay]
        """

        self._time_series_days = time_series_days

    @property
    def api_simulation(self):
        """Gets the api_simulation of this CreateSimulationRequest.  # noqa: E501


        :return: The api_simulation of this CreateSimulationRequest.  # noqa: E501
        :rtype: bool
        """
        return self._api_simulation

    @api_simulation.setter
    def api_simulation(self, api_simulation):
        """Sets the api_simulation of this CreateSimulationRequest.


        :param api_simulation: The api_simulation of this CreateSimulationRequest.  # noqa: E501
        :type: bool
        """

        self._api_simulation = api_simulation

    @property
    def is_time_travel(self):
        """Gets the is_time_travel of this CreateSimulationRequest.  # noqa: E501


        :return: The is_time_travel of this CreateSimulationRequest.  # noqa: E501
        :rtype: bool
        """
        return self._is_time_travel

    @is_time_travel.setter
    def is_time_travel(self, is_time_travel):
        """Sets the is_time_travel of this CreateSimulationRequest.


        :param is_time_travel: The is_time_travel of this CreateSimulationRequest.  # noqa: E501
        :type: bool
        """

        self._is_time_travel = is_time_travel

    @property
    def time_travel_start_date(self):
        """Gets the time_travel_start_date of this CreateSimulationRequest.  # noqa: E501


        :return: The time_travel_start_date of this CreateSimulationRequest.  # noqa: E501
        :rtype: str
        """
        return self._time_travel_start_date

    @time_travel_start_date.setter
    def time_travel_start_date(self, time_travel_start_date):
        """Sets the time_travel_start_date of this CreateSimulationRequest.


        :param time_travel_start_date: The time_travel_start_date of this CreateSimulationRequest.  # noqa: E501
        :type: str
        """

        self._time_travel_start_date = time_travel_start_date

    @property
    def time_travel_end_date(self):
        """Gets the time_travel_end_date of this CreateSimulationRequest.  # noqa: E501


        :return: The time_travel_end_date of this CreateSimulationRequest.  # noqa: E501
        :rtype: str
        """
        return self._time_travel_end_date

    @time_travel_end_date.setter
    def time_travel_end_date(self, time_travel_end_date):
        """Sets the time_travel_end_date of this CreateSimulationRequest.


        :param time_travel_end_date: The time_travel_end_date of this CreateSimulationRequest.  # noqa: E501
        :type: str
        """

        self._time_travel_end_date = time_travel_end_date

    @property
    def custom_aggregate_refineries(self):
        """Gets the custom_aggregate_refineries of this CreateSimulationRequest.  # noqa: E501


        :return: The custom_aggregate_refineries of this CreateSimulationRequest.  # noqa: E501
        :rtype: list[int]
        """
        return self._custom_aggregate_refineries

    @custom_aggregate_refineries.setter
    def custom_aggregate_refineries(self, custom_aggregate_refineries):
        """Sets the custom_aggregate_refineries of this CreateSimulationRequest.


        :param custom_aggregate_refineries: The custom_aggregate_refineries of this CreateSimulationRequest.  # noqa: E501
        :type: list[int]
        """

        self._custom_aggregate_refineries = custom_aggregate_refineries

    def to_dict(self):
        """Returns the model properties as a dict"""
        result = {}

        for attr, _ in six.iteritems(self.swagger_types):
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x.to_dict() if hasattr(x, "to_dict") else x,
                    value
                ))
            elif hasattr(value, "to_dict"):
                result[attr] = value.to_dict()
            elif isinstance(value, dict):
                result[attr] = dict(map(
                    lambda item: (item[0], item[1].to_dict())
                    if hasattr(item[1], "to_dict") else item,
                    value.items()
                ))
            else:
                result[attr] = value
        if issubclass(CreateSimulationRequest, dict):
            for key, value in self.items():
                result[key] = value

        return result

    def to_str(self):
        """Returns the string representation of the model"""
        return pprint.pformat(self.to_dict())

    def __repr__(self):
        """For `print` and `pprint`"""
        return self.to_str()

    def __eq__(self, other):
        """Returns true if both objects are equal"""
        if not isinstance(other, CreateSimulationRequest):
            return False

        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """Returns true if both objects are not equal"""
        return not self == other
